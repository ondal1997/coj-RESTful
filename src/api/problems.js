...
router.put("/problems/:key", async (req, res) => {
  ...
  if (Number.isNaN(problem.timeLimit) || Number.isNaN(problem.memoryLimit)) {
    console.log("400");
    res.json({ status: 400 });
    return;
  }
  if (
    problem.timeLimit < 200 ||
    problem.timeLimit > 5000 ||
    problem.memoryLimit < 128 ||
    problem.memoryLimit > 512
  ) {
    console.log("400");
    res.json({ status: 400 });
    return;
...
router.get("/problems/:key", async (req, res) => {
  ...
  try {
    ...
    problem.challengeCode = await getChallengeCode(problem.key, req.userId);
    problem.submitCount = await Solution.count({ problemKey: problem.key });
    problem.solvedCount = await Solution.count({
